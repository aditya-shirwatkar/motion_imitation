...
class TorqueStanceLegController(leg_controller.LegController):
  ...
  def get_action_rf(self):
    ...
    desired_com_roll_pitch_yaw[2] += desired_com_angular_velocity[0:2]*_PLANNING_HORIZON_STEPS*_PLANNING_TIMESTEP
    desired_rotation_matrix = Rotation.from_euler('xyz', desired_com_roll_pitch_yaw).as_matrix()
    ...
    Ud = np.zeros((12, _PLANNING_HORIZON_STEPS), dtype=np.float64)
    Xd[0:3, :] = np.linspace((0, 0, self._desired_body_height), desired_com_position, _PLANNING_HORIZON_STEPS).T
    desired_rotation_matrix_flat = desired_rotation_matrix.reshape(-1, order='F')
    Xd[6:15, :] = np.array([desired_rotation_matrix_flat for _ in range(_PLANNING_HORIZON_STEPS)]).T
    
    fz_unit = foot_contact_state*self._body_mass*9.8/(foot_contact_state.sum() + 1e-6)
    Ud[[2,5,8,11], :] = np.array([fz_unit for _ in range(_PLANNING_HORIZON_STEPS)]).T
    ######################################################
    ...
    com_rotation_matrix = Rotation.from_euler('xyz', com_roll_pitch_yaw)
    Xt = np.array([
            0, # com position x
            0, # com position y
            current_walking_height,
            body_vel_w[0],
            body_vel_w[1],
            body_vel_w[2],
            R_w2b[0,0], 
